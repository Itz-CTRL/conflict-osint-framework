...
import { Card, SectionTitle, StatusBadge, RiskBadge, EmptyState, Alert, Spinner, TabBar, Btn } from './UI';
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 24 }}>
        {[
          ...
          <div key={s.label} style={{
            ...
            <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: t.textSub, letterSpacing: 2, marginBottom: 6 }}>
              {s.label.toUpperCase()}
            </div>
            <div style={{ fontFamily: "'Sora',sans-serif", fontSize: 28, fontWeight: 800, color: t.text, lineHeight: 1 }}>
              {s.value}
            </div>
          </div>
        ))}
      ...
      {activeTab === 'investigations' && (
        <div className="animate-fadeIn">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <Btn variant="secondary" size="sm" theme={theme} onClick={refreshInvestigations} disabled={loadingList}>
                {loadingList ? <><Spinner size={12} color={t.textMuted} /> Refreshing</> : '↻ Refresh'}
              </Btn>
          ...
          {loadingList ? (
            ...
            <EmptyState icon="◎" message="No investigations yet. Launch your first scan." theme={theme} />
          ) : (
            <Card theme={theme} style={{ overflow: 'hidden' }}>
              ...
              <div style={{
                ...
                background: t.tableHeaderBg, borderBottom: `1px solid ${t.border}`,
                alignItems: 'center',
              }}>
                {['USERNAME', 'STATUS', 'RISK', 'CREATED', ''].map(h => (
                  <div key={h} style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, fontWeight: 700, color: t.textMuted, letterSpacing: 2 }}>
                    {h}
                  </div>
                ))}
              </div>
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{
          ...
          fontFamily: "'Sora',sans-serif", fontSize: 12, fontWeight: 700, color: t.accent, flexShrink: 0,
        }}>
        ...
        <div>
          ...
          <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: t.textMuted, marginTop: 1 }}>
            {inv.scan_type ? (inv.scan_type === 'deep' ? '🔬 deep' : '⚡ light') : ''}{inv.id ? `  ${inv.id.slice(0, 12)}…` : ''}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <button
          ...
          style={{
            padding: '5px 12px', background: `${t.accent}22`,
        ...
        <button
          ...
          disabled={deleting}
          style={{
            padding: '5px 9px', background: `${t.riskHigh}11`,
            border: `1px solid ${t.riskHigh}33`, borderRadius: 6,
            color: t.riskHigh, cursor: deleting ? 'not-allowed' : 'pointer',
